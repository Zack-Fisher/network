...
use crate::world_interaction::analysis::{AnalyseBundle, Analysis, AnalysisEvent, event::EventTypes};
...
)
{
    for (ent, prefab) in prefab_q.iter() {
        info!("spawning the analysis prefab from its analysis object {:?}", prefab.clone());
                ...
                children
                    .spawn(
                        AnalyseBundle {
                            ...
                            analysis: prefab.analysis.clone(),
                            ..default()
                        }
                    )
                    ;
