...
impl Plugin for LoadingPlugin {
    fn build(&self, app: &mut App) {
        app
            .add_plugin(RonAssetPlugin::<SerializedLevel>::new(&["lvl.ron"]))
...
pub struct SceneAssets {
    ...
    #[asset(path = "scenes/test.glb#Scene0")]
...
pub struct LevelAssets {
    //load the level pointers here so that we don't have to do weird handle stuff at runtime.
